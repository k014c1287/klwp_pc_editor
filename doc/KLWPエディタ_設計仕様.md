...
Treeviewにフォーカスがある状態のDeleteキーは `_on_delete_shortcut()` を経由して既存の `cmd_delete()` を呼びます。ツールバー操作と同じ確認ダイアログを表示し、確定後だけ兄弟コレクションから削除して履歴へ記録します。バインド範囲をTreeviewに限定するため、右ペインの文字入力中にDeleteを押してもアイテム削除は発生しません。
