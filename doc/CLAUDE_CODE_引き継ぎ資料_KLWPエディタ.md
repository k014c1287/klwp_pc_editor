...
- 100～400%の編集プレビューズーム、選択要素への自動フォーカス、ポインタ中心のCtrl+ホイール拡縮、操作中のキャッシュ画像による軽量追従、停止140ms後の高品質描画、ズーム後のドラッグ・リサイズ座標変換
...
- GUIなしの `render_to_image()` と回帰・構造テスト（現在64テスト）
...
- 全テスト: `python -m unittest -v`（機能63件＋構造1件）
