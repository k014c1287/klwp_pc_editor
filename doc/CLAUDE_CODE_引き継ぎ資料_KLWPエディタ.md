...
- GUIなしの `render_to_image()`、実機スクショとのMSE・PSNR・SSIM／差分ヒートマップ／品質ゲート、回帰・構造テスト（現在100テスト）
...
- 要素ツリーの矢印キーによる1単位、Shift+矢印キーによる10単位の座標微調整。複数選択とUndoに対応
...
- 100単位ルーラー、キャンバス／他要素の端・中心への4px候補ガイド。
  ドラッグ中は自由追従し、リリース時だけ最終補正（成果物へUI状態は保存しない）
...
- 全テスト: `python -m unittest -v`（機能99件＋構造1件）
...
9. 完了: 実機スクショとのピクセル差分ハーネス、ヒートマップ、MSE・PSNR・SSIM品質ゲート
10. 確認済み: OverlapLayerは既に全子要素を再帰的にwrapしている。
   `sizuka_home.klwp`で直接Shapeを持たない4レイヤーが既定
   `200x120`へフォールバックしないことを回帰テストとして固定
11. 確認済み: 2026-07-25報告の時計`11`逸脱・日付と天気テキスト重複は
   現行masterで再現せず。時刻文字の時計レイヤー内包と日付／天気の非重複を
   `sizuka_home.klwp`の回帰テストとして固定
12. 完了: Kode数式ライブエディタ。`text_expression` と
   `internal_formulas` を関数候補・構文診断・現在値評価付きで編集
13. 完了: FontIconピッカー。Material系定番アイコンとプリセット内の
   既存カスタムアイコンを検索グリッドから選択し、内蔵SVGとして保存
14. 完了: 要素の複数選択とコピー＆ペースト。別ファイル間では参照画像・
...
18. 完了: スナップ許容幅を4pxへ縮小し、ドラッグ中の強制吸着を廃止。
    候補ガイドを表示し、リリース時だけ補正する滑らかな操作へ変更
19. 完了: 要素ツリーの矢印キーで選択要素を1単位、Shift併用で10単位微調整。
    複数選択・アンカー／余白変換・Undoに対応し、連続操作中はツリーを再構築しない。
    選択後にプレビューへフォーカスが移っても操作でき、入力ウィジェットの矢印キーは奪わない

## 7.1 次の優先タスク候補 (2026-07-25 整理)

優先度は「PCプレビュー精度の向上」と「実機転送なしで完結できる編集体験」を
基準に付けた。上ほど投資対効果が高いと判断している。

### A. 精度・品質 (最優先)

1. **完了: 実機スクショとのピクセル差分ハーネス**
   `sample/` の実機スクショを正解画像として、`render_to_image()` 出力との
   差分ヒートマップ、MSE・PSNR・SSIM、品質ゲートを追加した。
2. **既知の描画グリッチ修正** (2026-07-25 sizuka_home で確認)
   - キャンバス最上部に浮く `11` (時刻数式の想定外描画と思われる)
   - 天気パネルで `32°C` と `July 22` 系テキストが重なる
   1のハーネスで再現を固定してから原因調査する。
3. **OverlapLayer サイズの union 算出** (§3-6 の近似改善)
   現状「最初の ShapeModule 子要素のサイズ」で近似 (`_layer_box_size`) 。
   子要素の描画境界 union に置き換えると、図形を持たないレイヤーの
   200x120 既定 (sizuka_home で4/18が該当) が解消する。
4. **§3 rule 3 (レイヤー内 offset) の実機再検証**
   要素種別依存の可能性を §3 に記載済み。実機がある環境で確定させる。

### B. 編集体験 (次点)

5. **Kode数式のライブエディタ**
   `text_expression` / `internal_formulas` は今 JSON 直接編集のみ。
   関数候補補完・構文チェック・その場評価プレビューを持つ専用パネルを作る。
6. **FontIcon の GUI 選択 (アイコンピッカー)**
...
9. **保存時の自動 adb 転送＋KLWP リロード「ライブ反映」**
10. **公式Kode の未対応関数の拡充**
11. **Android 3.82 Shader 対応**
12. **ADB経由のアプリ／Activity 選択支援**
13. **未知キー・未対応構文のバリデータ**

## 8. 開発方針・注意事項

- 開発ブランチ名は `BR_REVIEW_<やっていること>` 形式とし、全体を英大文字・数字・アンダースコアだけで構成する。詳細と例はルートのREADME「開発用ブランチ命名規則」を参照すること
