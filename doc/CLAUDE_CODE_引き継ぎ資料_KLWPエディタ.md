...
実プリセット `sizuka_home.klwp` を基準に確認中。
...
を配置。2026-07-20 にこの3ファイルを解析し、本資料と突き合わせ済み
(差分は §2/§3/§5/§7 に反映)。他プリセットでの検証に使えるサンプルとして
引き続き利用可能。
...
- GUIなしの `render_to_image()` と回帰・構造テスト（現在21テスト）

以降に残る「Bitmap/数式/影/回転が未実装」という記述は改修前の履歴であり、
上記が現行状態。未対応なのは主に全アニメーション状態、Komponent固有scale、
線形／放射グラデーション、blend mode、Kode全構文である。
...
3. **offset_x/y はルート直下の要素にのみ適用**。レイヤー内の子要素では
   **無視される** (例: dock 内 Stack の offset_x=62 は実機で効いていない)
   ⚠️**要再検証 (2026-07-20 反例あり)**: sample/ の3ファイル全てで、
   OverlapLayerModule の子 ShapeModule に非ゼロ offset が同一パターンで
   繰り返し設定されている実例を確認 (sizuka_home: 7箇所で offset (200,0)、
   genoblanc: 6箇所で offset (0,-91)/(1,-91))。単発の値ではなく複数要素で
   揃っているため偶然の残留値ではなく意図的な配置の可能性が高い。
   一方、元の反証だった「dock 内 StackLayerModule 自身」の offset_x=62 も
   sizuka_home に実在は確認できた (OverlapLayerModule "dock" の子である
   StackLayerModule "dock" 自身が offset (62,0) を持つが実機で無効、という
   のが元の観察)。→ **「レイヤー内の子要素は offset 無視」ではなく、
   「ShapeModule の offset はレイヤー内でも有効、StackLayerModule 自身が
   親レイヤー内で持つ offset は無効 (Stack は整列ロジックで位置決めされる
   ため offset が意味を持たない)」という、要素種別に依存する可能性が高い**。
   実機での再確認が必要 (このドキュメント作成環境には実機がないため未検証)
4. ルートの CENTER 系アンカー (CENTER/CENTERLEFT/CENTERRIGHT) では
   **Y オフセットは上方向が正** (実機観察による経験則。BOTTOM 系も上が正)
5. StackLayer の子は anchor/offset ではなく**順番に整列**
   (`config_margin` 間隔、HORIZONTAL_* なら横、それ以外は縦)
6. OverlapLayer 自体のサイズは明示されない → 現実装は
   「最初の ShapeModule 子要素のサイズ」で近似 (`_layer_box_size`)
7. FontIcon の既定 `icon_size` は **約80単位** (dock 実測から逆算)

※ ルール 3/4/7 は経験則。他プリセットで反例が出たら要再検証。
...
- 数式はサンプル値 (時刻 02:25、バッテリー95%、天気 32°C 等)。
  `internal_formulas` / globalsによる背景・色・幅・可視性等は評価するがKode全構文ではない
- OUTERグローは対応済み。線形／放射グラデーション、blend modeは未実装
- SCROLL移動・FADE、SWITCH移動、LOOP_2W移動/FADEは操作プレビュー対応済み。
  ROTATE/SCALE/色フィルター、複雑なeaseは未再現
...
- KomponentはOverlapとして子を描画するが、`config_scale_value`による内容全体の拡大率は未対応（旧形式の外側padding補正には利用）
...
メトリクス測位のため実害なし)。sample/ 全3ファイルの描画回帰も通過。

## 6. テスト・検証方法

- 全テスト: `python -m unittest -v`（機能20件＋構造1件）
...
1. Komponent `config_scale_value` の内容全体への適用
2. 線形／放射／Sweepグラデーションとblend mode
3. Kode評価器の構文・関数拡張とプレビュー値の編集UI
4. ROTATE/SCALE/色フィルター等のアニメーションとease拡張
5. Switch以外のグローバル変数 (`globals_list`) のエディタ UI
6. Androidアプリ・音楽等の外部タッチアクション作成 UI
7. adb ワンクリック転送 (`adb push → /sdcard/Kustom/wallpapers/`)
8. sampleを追加し、v10/v11/v15以外の配置・回転規則を回帰検証
